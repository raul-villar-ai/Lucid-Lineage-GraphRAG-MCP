...
* **The Tools:** Unified in `src/graph_tools.py`, consumed by both LangChain and MCP:
  ...
  * `audit_restricted_asset_leaks` — Full-graph scan detecting assets that cross compliance boundaries. The tool applies a canonical ordering (`c1.name < c2.name`, which also excludes the self-pair) so a single node governed by multiple boundaries is not mis-reported as a leak and each offending node pair is listed exactly once; results are reported as distinct affected assets. (The security-traffic-light scans in `src/graph_admin.py` derive the same distinct-asset set with the equivalent `c1 <> c2` identity guard — see §2, "Per-submission security posture".)
